...
const styles = `
  ...
  * { box-sizing: border-box; }
  ...
  th {
    ...
    color: var(--text900);
    padding: 6px 8px;
    border-bottom: 2px solid var(--text900);
    text-align: left;
...
export function renderPdfPacketHtml({ sections, dealTitle }) {
  const safeTitle = escapeHtml(dealTitle);

  const sectionsHtml = sections.map((section, idx) => `
    <section class="section">
      <div class="section-header">
        <div class="brand">Puget Sound Therapeutics</div>
        <h2>${escapeHtml(section.title)}</h2>
        ${idx === 0 && safeTitle ? `<p style="font-size: 13px; color: ${COLORS.text500}; margin: 4px 0 0 0;">${safeTitle}</p>` : ""}
