...
const ALLOWED_EMAIL_COLUMNS = "email, invited_by_email, invited_by_name";
const ALLOWED_EMAIL_COLUMNS_FALLBACK = "email, invited_by_email";

async function loadAllowedEmails(serviceClient) {
  const result = await serviceClient
    .from("allowed_emails")
    .select(ALLOWED_EMAIL_COLUMNS);

  if (!result.error) return result;
  if (!String(result.error.message || "").includes("invited_by_name")) return result;

  const fallback = await serviceClient
    .from("allowed_emails")
    .select(ALLOWED_EMAIL_COLUMNS_FALLBACK);
...
export async function getAdminAnalytics({ dealSlug }) {
  ...
  const [pageViewsResult, revokedResult, allowedResult, videoViewsResult, shareViewsResult, shareTokensResult] = await Promise.all([
    ...
    loadAllowedEmails(serviceClient),
    serviceClient
