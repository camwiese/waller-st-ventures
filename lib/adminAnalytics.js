...
export function buildAdminAnalytics({
  ...
  // Build per-email video engagement from both dataroom and share link views
  const videoByEmail = new Map();
  for (const row of [...videoViews, ...shareViews]) {
...
export async function getAdminAnalytics({ dealSlug }) {
  ...
  const [pageViewsResult, revokedResult, allowedResult, videoViewsResult, shareViewsResult] = await Promise.all([
    serviceClient
      .from("page_views")
