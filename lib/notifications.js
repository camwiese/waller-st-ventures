...
export async function notifyNewSession(
  ...
  const recipients = customRecipients && customRecipients.length > 0 ? customRecipients : getAdminEmails();
          ...
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 520px; margin: 0 auto; background: #ffffff;">
            ...
            <div style="padding: 28px; border-left: 1px solid ${EMAIL.border}; border-right: 1px solid ${EMAIL.border};">
              ...
              <div style="margin-top: 24px;">
                <a href="${adminUrl}/admin" style="display: inline-block; background: ${EMAIL.buttonBg}; color: ${EMAIL.buttonText}; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-size: 14px; font-weight: 600;">View Access</a>
              </div>
            </div>
...
export async function notifyHighIntent(investorEmail, intentScore, customRecipients = null) {
  const apiKey = process.env.RESEND_API_KEY;
  const recipients = customRecipients && customRecipients.length > 0 ? customRecipients : getAdminEmails();
  if (!apiKey || recipients.length === 0) return;
...
export async function notifyShareLinkView(investorEmail, contentType, customRecipients = null) {
  const apiKey = process.env.RESEND_API_KEY;
  const recipients = customRecipients && customRecipients.length > 0 ? customRecipients : getAdminEmails();
  if (!apiKey || recipients.length === 0) return;
  ...
  const isDeck = contentType === "deck";
  const subject = isDeck
    ? `Deck viewed: ${investorEmail} opened the pitch deck`
    : `Podcast played: ${investorEmail} is listening to the CEO interview`;
  const bodyText = isDeck
    ? `<strong>${safeEmail}</strong> just viewed the pitch deck via share link.`
    : `<strong>${safeEmail}</strong> started listening to the CEO interview podcast via share link.`;
  ...
  try {
        ...
        html: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 520px; margin: 0 auto; background: #ffffff;">
            ...
            <div style="padding: 28px; border-left: 1px solid ${EMAIL.border}; border-right: 1px solid ${EMAIL.border};">
              ...
              <p style="font-size: 14px; color: ${EMAIL.bodyMuted}; margin: 0 0 24px 0;">${escapeHtml(timeStr)}</p>
              <div style="margin-top: 24px;">
                <a href="${adminUrl}/admin" style="display: inline-block; background: ${EMAIL.buttonBg}; color: ${EMAIL.buttonText}; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-size: 14px; font-weight: 600;">View Access</a>
              </div>
            </div>
    ...
    if (!res?.ok) {
      console.error(`[notifyShareLinkView] Failed after retries for ${investorEmail}`);
    }
  } catch (err) {
    console.error("[notifyShareLinkView] Failed to send:", err?.message || err);
  }
}
